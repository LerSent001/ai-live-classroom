...
此文件描述计划接入的能力，不代表授权、用户扣费或分润已经上线。
...
- 授权回调地址：待确认实际部署域名后配置。建议路由为 `/api/tokenpay/callback`；该路由尚未实现，不能将仓库 URL 当作回调地址。

## 应用介绍与模型使用场景

AI 在线课堂是一款开源 AI 视频教学产品。用户输入学习主题，文本模型生成教学脚本和分镜，视频模型逐段生成讲解视频，并在可交互的 3D 教室内播放。支持中文输入、教师形象选择、30 秒初始课程和最多两次 10 秒续讲。

计划通过 TokenPay 钱包授权获取每位用户的专属 TokenDance API Key，由用户自己的账户余额支付教案和视频生成费用。未授权或额度不足时不提交新的生成请求，不回退到项目方私人模型 Key；已有视频回放不产生新的模型消费。
...
- 文本模型：以实时目录选择支持所需 JSON 输出能力的模型；当前直接调用的 Gemini 模型尚未确认在网关可用。
- 视频模型：候选 `minimax-h3-max`，适配 `minimax:video_generation_v2` 的提交、轮询与结果格式；尚未做真实生成验证。
